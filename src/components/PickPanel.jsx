    ...
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {availablePositions.map(pos => {
          ...
          return (
            <button key={pos} onClick={() => onSelectPos(pos)} disabled={!eligible}
              className={`px-4 py-1.5 rounded-lg text-sm border transition-all cursor-pointer active:scale-95 ${selectedPos === pos ? "border-blue-400/60 bg-blue-400/10 text-blue-300" : "border-white/10 text-white/50 hover:bg-white/5"} disabled:opacity-20 disabled:cursor-not-allowed`}>
              {pos}
            </button>
          );
        })}
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
          ...
          {eligibleForSelected.map(player => (
            <button key={player.name} onClick={() => onPickPlayer(player.name)}
              ...
              <div>
                <div className="text-sm font-medium text-white">{hoopIQ ? "???" : player.name}</div>
                {!hoopIQ
                  ? <div className="text-[11px] text-white/40 mt-0.5">{player.pts.toFixed(1)} pts · {player.reb.toFixed(1)} reb · {player.ast.toFixed(1)} ast · {player.stl.toFixed(1)} stl · {player.blk.toFixed(1)} blk</div>
                  : <div className="text-[11px] text-white/30 mt-0.5">Stats hidden — trust your gut</div>
                }
              </div>
              {!hoopIQ && <div className="text-sm font-semibold text-white/30 ml-4 shrink-0">{playerRating(player)}</div>}
            </button>
          ))}
        </div>
      )}
