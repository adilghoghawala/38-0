...
export function ResultScreen({ result, roster, onReset }) {
  const { wins, losses, totals, failed, record, grade, flavor, color } = result;
  ...
  return (
    <div className="space-y-4">
      <div ref={cardRef} className="bg-[#0f0f0f] border border-white/10 rounded-2xl p-6 text-center">
          ...
          {Object.keys(THRESHOLDS).map(k => {
            ...
            return (
              <div key={k} className="bg-white/5 rounded-xl p-3">
                ...
                <div className={`text-base font-semibold ${pass ? "text-green-400" : "text-red-400"}`}>
                  {totals[k].toFixed(1)}
                </div>
        ...
        {perfect && (
          <div className="mt-4 text-yellow-400/80 text-xs font-medium tracking-wide">
            Perfect. The goat lineup.
          </div>
        )}
      </div>
